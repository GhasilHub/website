...
const SimpleToggle = memo(function SimpleToggle({
  isMonthly,
  onToggle,
  t,
}: SimpleToggleProps) {
    ...
    <article className='card-elevated flex-1 gap-4 sm:gap-5 lg:gap-6 !p-4 sm:!p-6 lg:!p-8'>
      ...
      <div>
        <div className='text-text-heading text-lg sm:text-xl font-bold mb-2'>
          {t('plans.basic.currency')}
        </div>
        <div className='flex items-baseline gap-1 whitespace-nowrap'>
          <span className='text-text-heading text-3xl sm:text-4xl lg:text-5xl font-bold'>
            {plan.price}
          </span>
          <span className='text-text-heading text-sm sm:text-base lg:text-xl'>
            /{' '}
            {isAnnual
              ? t('plans.basic.yearly.period')
              : t('plans.basic.monthly.period')}
          </span>
        </div>
      </div>
      <p
        ...
        {plan.features.map((feature, index) => (
          ...
          >
            <Image
              ...
              alt={
                feature.available ? 'Included feature' : 'Not included feature'
              }
              width={20}
              height={20}
      ...
      <div>
        <button
          ...
          type='button'
          aria-label={`${t('buttons.getStarted')} with ${plan.label} plan`}
        >
...
function Pricing() {
  ...
  return (
    <section
      ...
      id='pricing'
      aria-label='GhasilHub pricing plans and subscription options'
    >
      ...
      <div className='px-4 sm:px-6 md:px-8 lg:px-12 xl:px-0'>
        <div
          className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-9 max-w-7xl mx-auto'
          role='list'
