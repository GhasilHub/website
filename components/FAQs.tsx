...
function FAQs({ locale }: FAQsProps) {
  ...
  return (
    <section
      ...
      id='faqs'
      aria-label='Frequently asked questions about GhasilHub'
    >
        ...
        >
          {faqItems.map(item => (
            <AccordionItem
              key={item.key}
              value={item.key}
              className='border-b border-gray-200 py-4 sm:py-6 lg:py-8'
            >
              <AccordionTrigger
                className={`flex justify-between items-center p-2 sm:p-3 lg:p-4 text-text-primary text-base sm:text-lg font-medium leading-5 hover:no-underline ${
              ...
              <AccordionContent className='px-4 sm:px-6 lg:px-10 pt-4 sm:pt-5 lg:pt-6 pb-6 sm:pb-7 lg:pb-8'>
                <p
                  className={`text-text-secondary leading-8 ${
